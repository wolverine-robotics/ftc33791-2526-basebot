...
@Autonomous()
public class CloseBlueSidev2 extends OpMode {
    ...
    public void buildPaths() {
        ...
        StartToShoot.setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(135));
...
//                new Pose(48.715, 51.336),
        ...
        ShootFarLine.setLinearHeadingInterpolation(Math.toRadians(180), Math.toRadians(135));

        Park = new Path(new BezierLine(new Pose(shootPositionXCoordinate, shootPositionYCoordinate), new Pose(25, shootPositionYCoordinate)));
...
//                    setShooterVelTakeBackHalf(shooterVelocityGoal);
    ...
    public double shooterTakeBackHalf(double targetVel, double currentVel, double lastPower) {
        ...
        boolean atOrAboveTarget = currentVel >= targetVel - CloseRedSidev2.TeleOpConstants.TBH_TOLERANCE;

        if (atOrAboveTarget) {
            double half = lastPower * CloseRedSidev2.TeleOpConstants.TBH_HALF_FACTOR;
            return Math.max(CloseRedSidev2.TeleOpConstants.TBH_MIN_POWER, Math.min(1.0, half));
        } else {
            return Math.min(1.0, CloseRedSidev2.TeleOpConstants.TBH_FULL_POWER);
        }
    }
    ...
    public boolean shooterWithinTolerance(double target) {
        return withinTolerance(getAvgShooterVel(), target, CloseRedSidev2.TeleOpConstants.SHOOTER_TOLERANCE);
    }
    ...
    public boolean withinDistance() {
        return distance.getDistance(DistanceUnit.INCH) < CloseRedSidev2.TeleOpConstants.ACTIVATION_DISTANCE;
    }

    public void intakePassiveIndex() {
        intake.setPower(CloseRedSidev2.TeleOpConstants.INTAKE_POWER);
        index.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        index.setVelocity(CloseRedSidev2.TeleOpConstants.PASSIVE_INDEX_VELOCITY);
    }
    ...
    public void magDump(double seconds) {
        ...
        while (actiontime.seconds()<seconds) {
            index.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
            index.setPower(CloseRedSidev2.TeleOpConstants.MAG_DUMP_POWER);
            intake.setPower(CloseRedSidev2.TeleOpConstants.MAG_DUMP_POWER);
        }
    }
    ...
    public double getDistanceToTag() {
        ...
        if (result != null && result.isValid()) {
            ...
            double angle = Math.toRadians(CloseRedSidev2.TeleOpConstants.LIMELIGHT_MOUNT_ANGLE + ty);
            return (CloseRedSidev2.TeleOpConstants.GOAL_HEIGHT - CloseRedSidev2.TeleOpConstants.LIMELIGHT_HEIGHT) / Math.tan(angle);
        }
