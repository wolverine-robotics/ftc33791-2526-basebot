 ...
 */
@TeleOp
public class Teleop_Basebotv2 extends LinearOpMode {
    ...
    public static class TeleConstants {
        ...
        /** Take-back-half: power applied when below target (ticks/s). */
    ...
    public static Pose endOfAutoPose;
    ...
    private Pose currentAimedGoalPose;
    private final Pose blueGoalPose = new Pose(3, 142);
    private final Pose redGoalPose = new Pose(141, 142);
    // Gamepad state
    Gamepad gamepad;
//    Gamepad lastGamepad;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        follower.setStartingPose(endOfAutoPose != null ? endOfAutoPose : new Pose());
        initializeHardware();

        // Initialize lookup table
        shooterVelocityTable = new ShooterVelocityLookupTable(TeleConstants.DISTANCES, TeleConstants.VELOCITIES);
...
//        lastGamepad = new Gamepad();
        ...
        setShooterVelTakeBackHalf(TeleConstants.CLOSE_ZONE_VELOCITY);
        closeZone = true;
        ...
        while (opModeIsActive()) {
            ...
            pivot = gamepad.right_stick_x * TeleConstants.PIVOT_MULTIPLIER;
            heading = pinpoint.getHeading(AngleUnit.RADIANS);

            if (gamepad.cross) {
                telemetry.addData("Auto Align Status: ", autoAlign(TeleConstants.AUTO_ALIGN_TOLERANCE));
                double predictedVel = shooterVelocityTable.predict(getDistanceToTag());
                setShooterVelTakeBackHalf(predictedVel + TeleConstants.SHOOTER_PLUS);
            } else {
            ...
            if (gamepad.dpadUpWasPressed()) {
                closeZone = true;
                farZone = false;
                TeleConstants.MAG_DUMP_POWER = 0.8;
                setShooterVelTakeBackHalf(TeleConstants.CLOSE_ZONE_VELOCITY);
            } else if (gamepad.dpadDownWasPressed()) {
                farZone = true;
                closeZone = false;
                TeleConstants.MAG_DUMP_POWER = 0.7;
                setShooterVelTakeBackHalf(TeleConstants.FAR_ZONE_VELOCITY);
            }

            if (getDistanceToTag() > 40 && getDistanceToTag() != Double.POSITIVE_INFINITY) {
                TeleConstants.MAG_DUMP_POWER = 0.6;
                TeleConstants.SHOOTER_PLUS = 50;
            } else {
                TeleConstants.MAG_DUMP_POWER = 0.9;
                TeleConstants.SHOOTER_PLUS = 85;
            }

            // --- SHOOTER (take-back-half runs every loop when shooter target > 0) ---
            if (shooterTargetVel > 0) {
                setShooterVelTakeBackHalf(shooterTargetVel);
            }
            ...
            // --- INTAKE ---
            // --- INDEX ---
            if (gamepad.right_trigger > TeleConstants.TRIGGER_THRESHOLD) {
                intake.setPower(TeleConstants.INTAKE_POWER);
                index.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
                index.setVelocity(TeleConstants.PASSIVE_INDEX_VELOCITY);
            } else if (gamepad.left_trigger > TeleConstants.TRIGGER_THRESHOLD) {
                intake.setPower(TeleConstants.INTAKE_REVERSE_POWER);
            } else if (gamepad.dpadRightWasPressed()) {
                setIndexPos(index.getCurrentPosition() + TeleConstants.INDEX_STEP);
                intake.setPower(1.0);
...
//                intake.setPower(1.0);
                ...
                index.setPower(TeleConstants.MAG_DUMP_POWER);
                intake.setPower(TeleConstants.MAG_DUMP_POWER);
            } else   {
                index.setPower(1.0);
...
//            telemetry.addData("distanceSensor", distance.getDistance(DistanceUnit.INCH));
    ...
    public void fieldCentric(double y, double x, double rx, double heading) {
        ...
        rotX = rotX * TeleConstants.STRAFE_CORRECTION;
    ...
    public void poseAutoAlign() {
        Pose displacement = currentAimedGoalPose.minus(follower.getPose());
        double targetAngleDeg = Math.toDegrees(Math.atan2(displacement.getY(), displacement.getX()));
        double error = targetAngleDeg - Math.toDegrees(follower.getHeading());
        double omega = Math.max(-1.0, Math.min(1.0, TeleConstants.ALIGN_P_GAIN * error));

        fieldCentric(0, 0, omega, 0);
    }
    ...
    public String autoAlign(double tolerance) {
        ...
        double tx = result.getTx() - (getDistanceToTag() > 40 ? TeleConstants.TX_OFFSET_DEGREES_CLOSE : TeleConstants.TX_OFFSET_DEGREES_FAR);
        ...
        double pivot = Math.max(-1.0, Math.min(1.0, TeleConstants.ALIGN_P_GAIN * tx));
    ...
    /** Take-back-half: when at/above target use half of last power; when below use full power. */
    public double shooterTakeBackHalf(double targetVel, double currentVel, double lastPower) {
        if (targetVel <= 0) return 0;

        boolean atOrAboveTarget = currentVel >= targetVel - TeleConstants.TBH_TOLERANCE;

        if (atOrAboveTarget) {
            double half = lastPower * TeleConstants.TBH_HALF_FACTOR;
            return Math.max(TeleConstants.TBH_MIN_POWER, Math.min(1.0, half));
        } else {
            return Math.min(1.0, TeleConstants.TBH_FULL_POWER);
        }
    }

    /** Set shooter target velocity and apply TBH power this cycle. Call every loop when shooter is on. */
    ...
    public boolean shooterWithinTolerance(double target) {
        return withinTolerance(getAvgShooterVel(), target, TeleConstants.SHOOTER_TOLERANCE);
    }
    ...
    public double shooterPID(double targetVel, double currentVel) {
        ...
        double proportional = TeleConstants.SHOOTER_P_GAIN * error;

        shooterIntegral += error * 0.02; // Approximate dt
        double integralTerm = TeleConstants.SHOOTER_I_GAIN * shooterIntegral;

        double derivative = (error - shooterLastError) / 0.02;
        double derivativeTerm = TeleConstants.SHOOTER_D_GAIN * derivative;
...
//    }
    ...
    public double getDistanceToTag() {
        ...
        if (isLatestResultValid()) {
            ...
            double angle = Math.toRadians(TeleConstants.LIMELIGHT_MOUNT_ANGLE + ty);
            return (TeleConstants.GOAL_HEIGHT - TeleConstants.LIMELIGHT_HEIGHT) / Math.tan(angle);
        }
        return 9999;
    }
    ...
    public static class ShooterVelocityLookupTable {
        ...
        public double predict(double distance) {
            if (Double.isNaN(distance) || Double.isInfinite(distance)) return TeleConstants.CLOSE_ZONE_VELOCITY;
            if (distances == null || velocities == null) return TeleConstants.CLOSE_ZONE_VELOCITY;
            if (distances.length == 0 || velocities.length == 0) return TeleConstants.CLOSE_ZONE_VELOCITY;
            if (distances.length != velocities.length) return TeleConstants.CLOSE_ZONE_VELOCITY;
    ...
    void initializeHardware() {
        ...
        pinpoint.setOffsets(TeleConstants.PINPOINT_X_OFFSET, TeleConstants.PINPOINT_Y_OFFSET, DistanceUnit.MM);
